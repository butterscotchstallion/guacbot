...
parser.parseDuration = function (input) {
    ...
    var durationDigitLength = duration.toString().length;
    var isValidDuration     = !isNaN(duration);
    var unitIsMinutes       = unit === 'm';
    var durationDigitLimit  = 2;
    
    // Issue #110 - allow three digit durations for minutes
    if (unitIsMinutes) {
        durationDigitLimit = 3;
    }
    
    isValidDuration  = durationDigitLength <= durationDigitLimit;
