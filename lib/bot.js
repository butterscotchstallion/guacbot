...
var client = new irc.Client(config.server, config.nick, {
    ...
    showErrors: true,
    userName: config.userName || 'nodebot',
    realName: config.realName || 'nodebot',
    stripColors: true,
...
// Initialize plugins
var enabledPlugins = Object.keys(config.plugins);
...
console.log('Loading plugins (' + epLen + ')...');
console.log('===========================================');

for (var j = 0; j < epLen; j++) {
    plugin = '../plugins/' + enabledPlugins[j];
    
    console.log('Loading plugin: ' + plugin);
    
    require(plugin).init(client, config);
}
